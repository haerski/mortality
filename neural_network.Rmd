...
sln_fit %>% 
  collect_metrics()
```
# Penalized Nerual Network Model 2 
```{r}
sln_2_spec <-
  mlp(penalty=tune()) %>%
  set_engine("nnet") %>%
...
sln_2_workflow <- 
  workflow() %>%
  add_model(sln_2_spec) %>% 
  add_recipe(sln_recipe)
```

# we need to choose the penalty (furter split training into training and validation or do cross validation )
```{r}
sln_val <- validation_split(training(sln_split))
sln_val
```

# Now we want to tune
```{r}
sln_penalty <- grid_regular(penalty(), levels=100)
sln_2_tune <-
  sln_2_workflow %>%
  tune_grid(sln_val,
            grid = sln_penalty,
...
sln_2_tune %>%
  collect_metrics() %>%
  filter(.metric == "sens") %>%
  ggplot(aes(x = penalty, y = mean)) + geom_line() + scale_x_log10() + geom_vline(xintercept = 1.023531e-09)
...
```{r}
sln_2_tuned_workflow <- sln_2_workflow %>% 
  finalize_workflow(parameters=list(penalty=1.023531e-09))

sln_2_tuned_fit <- reg_2_tuned_workflow %>%
...
```{r}
collect_metrics(sln_2_tuned_fit)
collect_metrics(sln_fit)
```
