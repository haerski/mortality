...
We explain how we obtained the dataframe weekly_data.

To begin with, we used the following 3 data sets:
...
We take daily covid deaths (`data/deaths_zip3.feather`) and make them weekly. We look at weekly deaths instead of deaths to date.
```{r}
deaths <-
  ...
  ungroup()
```
One thing to note is that there is no death data for zipcodes 202, 204, 753, 772, which is fine since there is no zip_data for those either.

Next we look at the deaths only. We created a tibble `per` containing all the people that die and their deathweek. Everyone should die only once. 
```{r}
per %<>%
  drop_na() %>%
  select(-month)
...
2019 didn't have 53 weeks, so we change the deaths on week 53 in 2019 to death in week 1 of 2020. 
...
We convert the week and year into the last date of that week.
```{r}
per %<>%
  nest_by(week, year) %>%
...
Next, we need some kind of a rolling count for AE. We use the package `slider`.

We want actual claims per week for each client. We note that there are 4 clients that won't have any deaths. We compute face amount per week for each client. This number is 0 if the client has no deaths that week. We put this information into the tibble `weekly_claims`. 
```{r}
weekly_claims <-
  per %>%
...
Then we merge everything to get a tibble called 'weekly_data'. This contains the following information for each week (from 2019-01-06 from 2021-06-27), each client:
1. claims amount for each week (time dependent)
2. Zip death for each week (time dependent)
3. Weekly ae ( ae = claims / (expected / 52.18) )
4. Zip3 information and client characteristic (non time dependent)
```{r}
weekly_data <-
  clients %>%
...
Then we shrink smoothed ae based on the $log(Volume * average q_x)$. This gives us some kind of a measure of client size and mortality. We add the shrunk smoothed AE to `weekly_data` to get the tibble `processed_data`. 
```{r}
client_shrinkage <-
  weekly_data %>%
...
We compute the 25th percentile of Shrunk smoothed AE of each week. We choose `smoothed shrunk AE > 2.5` as "Adverse". (2.5 is close to the 25th percentile of the weekly shrunk smoothedAE over all time). We add this information to the tibble `processed_data`. 
...
```
Now processed_data contains the following:
1. claims amount for each week (time dependent)
2. Zip death for each week (time dependent)
3. Smoothed AE and smoothed weekly zip deaths 
4. Shrunk smoothed AE 
5. Class (adverse or not)
6. Weekly AE ( ae = claims / (expected / 52.18) )
7. Zip3 information and client characteristic (non time dependent)


Since we will also use IHME forecasts (more trustworthy than our naive ARIMA),we will need some additional data wrangling.
...
We augment the `processed_data` by adding the state name
```{r}
zip_to_state <-
...
Next we read the data from IHME as of Dec 23 2020 and add it to `processed_data`
...
```
Now the `processed_data` contains the following information:

We save `processed_data` in `data/processed_data_20_12_23.feather`.
```{r}
write_feather(processed_data, "data/processed_data_20_12_23.feather")
```

Removing some columns which we wouldn't use in prediction, we get our main data `weekly_data` for the short term models.
```{r}
weekly_data <-
  read_feather("data/processed_data_20_12_23.feather") %>%
  select(-ae_2021, -ae_2020, -ae_2019,
...
We pick client `7`, `10`, `61`, `100` as examples. We add the solid line `threshold = 2.5` to help us to see whether this client meet adverse mortality event.The exact AE is in the range of confidence interval. 
```{r warning = FALSE}
result %>%
    group_by(client) %>%
...
After evaluating several metrics of different machine learning models for the long term version, we choose the Random Forest and we tune its hyperparameters. We aim at minimizing the proportion of adverse clients that are incorrectly predicted as not adverse since this number causes huge money loss for Securian. We also aim at minimizing the proportion of not adverse clients that are predicted as adverse, since this causes clients loss and hence again money loss. After choosing the best parameters, and choosing a threshold, we are able to reach a sensitivity of 85%, a specificity of 78% and an accuracy of 82%. We then use the SHAP values to increase the transparency of the model and understand the contribution of the predictors in the classification of our clients. 
We then include the time factor in the short term model and we aim at using some known clients' performance in the past to predict unknown clients performance in the future. For this version, we use the rolling AE (updated weekly) as opposed to using yearly AE in the long term version. We also add the weekly deaths by covid to the list of predictors used in the long term model. One key step in the process is forecasting the weekly deaths to use it in the future. For this time-dependency version, we choose the Boosted Trees model after comparing its performance with other known machine learning models. After tuning it and choosing the best parameters, we present the accuracy of the predictions for unknown clients and compare it with the one for the known clients. 
