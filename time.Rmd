...
# foo %>%
#   autoplot(train %>% bind_rows(test) %>% ungroup() %>% as_tsibble(index = date, key = client))
foo <-
  forecast %>%
  index_by(date) %>%
...
forest_recipe <-
  ...
  step_rm(client, zip3, claims, smoothed_ae, shrunk_ae, shrinkage, dep_var, ae) %>%
...
trained_wf %>%
  predict(forecasted_test) %>%
  bind_cols(test) %>%
  group_by(date) %>%
  summarize(sens = sens_vec(class, .pred_class), spec = spec_vec(class, .pred_class)) %>%
  pivot_longer(sens:spec, names_to = "metric", values_to = "value") %>%
  ggplot(aes(x = date, y = value, color = metric)) + geom_line()
