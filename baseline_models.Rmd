...
models <- list(log = log_spec,
               ...
               knn_spec = knn_spec)
recipes <- list("with_2019_ae" = with2019_rec,
                "no_2019_ae" = no2019_rec)
...
fit_wflows %>%
  collect_metrics() %>%
  mutate(with = str_detect(wflow_id, "with")) %>%
  ggplot(aes(x = model, y = mean, shape = with)) +
  geom_line() +
  facet_wrap(~ .metric)
```
...
tuned_svm <-
  results %>%
  pull_workflow_set_result("normalized_svm")
